...
export default function RoleSelectionScreen() {
  ...
  const { userRole, setUserRole, authStep } = useUserStore()
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>Electric Vehicle Architecture Training System</Text>
        ...
        <View style={styles.buttonGroup}>
          {ROLE_CONFIG.map(({ role, label, icon, color, colorRgb }) => (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.roleButton,
                { borderColor: color, backgroundColor: 'rgba(' + colorRgb + ', 0.08)' },
...
const styles = StyleSheet.create({
  ...
  eyebrow: { fontFamily: 'SpaceMono', fontSize: 13, fontWeight: '600', letterSpacing: 3, textTransform: 'uppercase', color: '#67e8f9', marginBottom: 20, textAlign: 'center' },
  title: { fontFamily: 'SpaceMono', fontSize: 30, fontWeight: '700', color: '#f8fafc', textAlign: 'center', lineHeight: 38, marginBottom: 28 },
  prompt: { fontFamily: 'SpaceMono', fontSize: 12, fontWeight: '600', letterSpacing: 2, textTransform: 'uppercase', color: '#9debf4', marginBottom: 48, textAlign: 'center' },
  buttonGroup: { width: '100%', gap: 14 },
  roleButton: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 10, paddingVertical: 18, paddingHorizontal: 20 },
  roleButtonPressed: { opacity: 0.75, transform: [{ scale: 0.98 }] },
  roleIcon: { fontSize: 22, marginRight: 14 },
  roleLabel: { fontFamily: 'SpaceMono', fontSize: 17, fontWeight: '700', letterSpacing: 1 },
  backLink: { fontFamily: 'SpaceMono', fontSize: 12, color: '#9debf4', marginTop: 28, textDecorationLine: 'underline' },
